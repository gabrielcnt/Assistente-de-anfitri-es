...
def criar_dica(imovel_id: int, dica: DicasLugaresSchemaCreate, db: Session = Depends(get_db)):
    ...
    try:
        return service.criar_dica_lugar(user_id, imovel_id, dica)
...
def listar_dicas_por_imovel(imovel_id: int, db: Session = Depends(get_db)):
    ...
    return service.listar_dicas(user_id, imovel_id)



@router.patch("/imoveis/{imovel_id}/dicas/{dica_id}", response_model=DicasLugaresSchemaResponse)
def atualizar_dica(dica_id: int, imovel_id: int, dica_update: DicasLugaresSchemaUpdate, db: Session = Depends(get_db)):
    try:
        repo_dica = DicasLugaresRepository(db)
        repo_imovel = ImovelRepository(db)
        service = DicaLugarService(db, repo_dica, repo_imovel)
        ...
        return service.atualizar_dica(user_id, imovel_id, dica_id, dica_update)
...
def deletar_dica(imovel_id: int, dica_id: int, db: Session = Depends(get_db)):
    ...
    return service.apagar_dica(dica_id, imovel_id, user_id)